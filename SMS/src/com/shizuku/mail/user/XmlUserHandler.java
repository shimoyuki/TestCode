...
public class XmlUserHandler extends AbstractMailUserHandler {
	 ...
	 *      emailAddress, String password, String id)
	 */
	 ...
	 * 
	 * @see com.shizuku.mail.user.MailUserHandler#checkUser(String user,String
	 *      domain)
	 */
	...
	/**
	 * @see com.shizuku.mail.user.MailUserHandler#checkUser(EmailAddress
	 *      emailAddress)
	 */
		...
		for (Iterator<Element> i = list.iterator(); i.hasNext();) {
			...
			if (e.getChild("Name").getText().equals(userName)) {
				User user = new User(userName,
						e.getChild("Password").getText(), domain);
				return user;
			}
		}
		return null;
	 ...
	 * @param path
	 *            The xml file path.
	 * @return If save successfully then return true.
	 */
